...
<section>
    <span class="card-kicker block mb-3">Overview</span>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">

        <div class="card elev-sm gap-2.5">
            <div class="flex items-center justify-between">
                <span class="card-kicker">Total Workouts Logged</span>
        ...
        <div class="card elev-sm gap-2.5">
            <div class="flex items-center justify-between">
                <span class="card-kicker">Personal Records This Month</span>
        ...
        <div class="card elev-sm gap-2.5">
            <div class="flex items-center justify-between">
                <span class="card-kicker">Current Streak</span>
            ...
            <div class="text-4xl font-semibold leading-none" style="font-family: var(--font-heading)">
                {{ $currentStreak }} <span class="text-base font-normal opacity-60">days</span>
            </div>
        </div>

        <div class="card elev-sm gap-2.5">
            <div class="flex items-center justify-between">
                <span class="card-kicker">Favorite Exercise</span>
...
<section class="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4 items-start">
    ...
    <div id="quick-add" class="card elev-sm p-5 gap-3.5 scroll-mt-24">
        ...
        <form method="POST" action="{{ route('records.store') }}" class="flex flex-col gap-3.5">
            @csrf
            <div class="field">
                <label for="qa-exercise">Exercise</label>
                <input class="input" id="qa-exercise" name="exercise" list="exercise-options" value="{{ old('exercise', $selectedExercise?->name) }}" required>
            ...
            <div class="grid grid-cols-3 gap-2.5">
                <div class="field"><label for="qa-weight">Weight (lb)</label><input class="input" id="qa-weight" name="weight" type="number" step="0.5" min="0" placeholder="185" value="{{ old('weight') }}" required></div>
                <div class="field"><label for="qa-reps">Reps</label><input class="input" id="qa-reps" name="reps" type="number" min="1" placeholder="5" value="{{ old('reps') }}" required></div>
                <div class="field"><label for="qa-set">Set</label><input class="input" id="qa-set" name="set_number" type="number" min="1" placeholder="1" value="{{ old('set_number', 1) }}" required></div>
            </div>
            <div class="field"><label for="qa-date">Date</label><input class="input" id="qa-date" name="date" type="date" value="{{ old('date', now()->toDateString()) }}" required></div>
...
<section class="card elev-sm p-5 gap-3.5">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <div class="card-title">Weight &amp; reps progression</div>
        </div>
        <form method="GET" action="{{ route('dashboard') }}">
            <select class="input" style="width:auto;min-width:180px" name="exercise" onchange="this.form.submit()">
                @foreach ($allExercises as $ex)
                    <option value="{{ $ex->name }}" @selected($selectedExercise?->id === $ex->id)>{{ $ex->name }}</option>
                @endforeach
            </select>
        </form>
    </div>
    ...
    @if ($selectedExercise && count($chartLabels) > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @include('partials.chart', ['title' => 'Weight (lb)', 'chart' => $weightChart, 'labels' => $chartLabels, 'color' => 'var(--color-accent)', 'areaColor' => 'var(--color-accent-soft)'])
            @include('partials.chart', ['title' => 'Reps', 'chart' => $repsChart, 'labels' => $chartLabels, 'color' => 'var(--color-accent-2)', 'areaColor' => 'var(--color-accent-2-soft)'])
        </div>
    @else
        <p class="card-body">Log a few sets to see progression charts here.</p>
    @endif
</section>

<section>
    <div class="flex items-center justify-between mb-3.5">
        <span class="card-kicker">Your exercises</span>
    </div>
